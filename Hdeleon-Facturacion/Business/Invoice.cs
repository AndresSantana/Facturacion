    ...
    public class Invoice
    {
        ...
        private void GetDataPDF()
        {
            using (Models.SAT.satEntities db2 = new Models.SAT.satEntities()) {
                //regimen fiscal
                factura.sRegimenFiscal = db2.f4_c_regimenfiscal.Where(d => d.c_RegimenFiscal == factura.RegimenFiscal).First().Descripción;
                //forma pago
                factura.sFormaPago = db2.f4_c_formapago.Where(d => d.c_FormaPago == factura.FormaPago).First().Descripción;
                //forma pago
                factura.sTipoDePago = db2.f4_c_metodopago.Where(d => d.c_MetodoPago == factura.TipoDePago).First().Descripción;
                //usocfdi
                factura.sUsoCFDI = db2.f4_c_usocfdi.Where(d => d.c_UsoCFDI == factura.UsoCFDI).First().Descripción;
            }
